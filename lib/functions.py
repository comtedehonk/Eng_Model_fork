...
class functions:
    ...
    def send_state_of_health(self, health_type):
        ptp = APTP(self.cubesat.radio1, packet_size=245, timeout=13.7, log=False)
        health_data = [
                health_type,
                f"PM:{c.power_mode}",
                f"VB:{c.battery_voltage}",
                f"ID:{c.current_draw}",
                f"IC:{c.charge_current}",
                f"VS:{c.system_voltage}",
                f"UT:{c.uptime}",
                f"BN:{c.c_boot}",
                f"MT:{c.micro.cpu.temperature}",
                f"RT:{c.radio1.former_temperature}",
                f"AT:{c.internal_temperature}",
                f"BT:{f.last_battery_temp}",
                f"AB:{int(c.burned)}",
                f"BO:{int(c.f_brownout)}",
                f"FK:{int(c.f_fsk)}"
            ]
        packet = Packet.make_health_packet(health_data)
        asyncio.run(ptp.send_packet(packet))
